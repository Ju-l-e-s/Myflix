...
def test_purge_share_directory_success(mock_paths, mocker):
    ...
    def side_effect_lstat(path):
        if 'old_file.mkv' in path: return mock_stat_old
...
def test_auto_tiering_nvme_to_hdd_not_needed(mock_paths, mocker):
    ...
    mocker.patch('shutil.disk_usage', return_value=MagicMock(total=100, used=20, free=80))
...
def test_auto_tiering_migration_flow(mock_paths, mocker):
    ...
    mocker.patch('shutil.disk_usage', side_effect=[
    ...
    mocker.patch('os.walk', return_value=[('/tmp/nvme/Movies/Titanic', [], ['titanic.mkv'])])
    ...
    mock_move.assert_called_once_with('/tmp/nvme/Movies/Titanic/titanic.mkv', '/tmp/hdd/Movies/Titanic/titanic.mkv')
    mock_symlink.assert_called_once_with('/tmp/hdd/Movies/Titanic/titanic.mkv', '/tmp/nvme/Movies/Titanic/titanic.mkv')
...
def test_auto_tiering_rollback_on_failure(mock_paths, mocker):
    import cleanup_share
    mocker.patch('shutil.disk_usage', return_value=MagicMock(total=100, used=80, free=20))
    mocker.patch('cleanup_share.is_hdd_mounted', return_value=True)
    ...
    # Rollback mocks
    mocker.patch('os.path.exists', side_effect=lambda p: p == '/tmp/hdd/Movies/fail.mkv')
    mock_rollback_move = mocker.patch('shutil.move') # Will be called again for rollback
...
def test_ensure_hdd_symlinks_creates_missing_link(mock_paths, mocker):
    ...
    mocker.patch('os.path.exists', side_effect=lambda p: p in ['/tmp/hdd/Films', '/tmp/hdd/Series'])
    mocker.patch('os.makedirs')
    
    # Mock contenu du HDD (un film présent)
    mocker.patch('os.listdir', side_effect=lambda p: ['Zootopia_2_2025'] if 'Films' in p else [])
    ...
    mock_symlink.assert_called_once_with(
        '/tmp/hdd/Films/Zootopia_2_2025',
        '/tmp/nvme/movies/Zootopia_2_2025'
    )
