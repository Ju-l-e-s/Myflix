...
CONFIG_FILE = (
    ...
    if DOCKER_MODE
    else os.path.join(BASE_DIR, "config_share.json")
)
...
def generate_secure_link(source_path, remux=False):
    ...
    if size_gb > config.get("max_gb", 4.0) or height > config.get("max_res", 1080):
        logging.warning(
            f"Limits exceeded: {size_gb}GB > {config.get('max_gb')}, {height}p > {config.get('max_res')}"
        )
        return None
    ...
    if remux and subtitles:
        ...
        if remux_subtitles(src, subtitles, video_dest_path):
            return {
                "video": f"{current_base_url}/{video_share_name}",
                "type": "remuxed",
            }
