...
def test_generate_secure_link_success(mock_config_file, share_dir, tmp_path, mocker):
    ...
    mocker.patch(
        "share_engine.DOCKER_MODE", True
    )  # Set to True to skip path translation logic for simple test
...
def test_generate_secure_link_limit_exceeded(
    mock_config_file, share_dir, tmp_path, mocker
):
    import share_engine
