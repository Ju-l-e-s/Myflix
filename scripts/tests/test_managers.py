...
def test_clean_media_name():
    ...
    assert media_manager.clean_media_name("Inception.2010.1080p.BluRay.x264.mkv") == "Inception (2010)"
    assert media_manager.clean_media_name("The.Boys.S01E01.720p.WEB-DL.mkv") == "The Boys"
    assert media_manager.clean_media_name("Avatar.The.Way.Of.Water.2160p.4k.HEVC.mkv") == "Avatar The Way Of Water"
...
def test_find_local_media(mocker):
    ...
    mocker.patch('os.listdir', side_effect=lambda p: ['Titanic.1997.mkv'] if 'Films' in p else [])
