...
def parse_allowed_ids(ids_str):
    ...
    for i in ids_str.split(","):
        ...
        if stripped and stripped.isdigit():
            ...
            logging.getLogger("Gatekeeper").error(
                f"ID invalide dans TELEGRAM_ALLOWED_USER_IDS : '{stripped}'"
            )
    return ids
...
logging.getLogger("Gatekeeper").info(
    f"Gatekeeper configuré avec les IDs : {ALLOWED_USER_IDS}"
)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(search_url, timeout=15) as resp:
                ...
                def score(item):
                    s = 0
                    if item["title"].lower() == clean_query.lower():
                        s += 100
                    if target_year and item.get("year") == target_year:
                        s += 50
                    return s
        ...
        payload["seasons"] = [
            {
                ...
                "monitored": s["seasonNumber"] == target_season,
            }
...
async def process_ai_logic(prompt):
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "Media assistant. Intent: movie|series|chat. JSON {type, query, reply?}. 'reply' only if chat.",
            },
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Salut ! Je suis ton assistant média. Envoie-moi un nom de film ou série."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if intent["type"] in ["movie", "series"]:
        ...
        if not results:
            await update.message.reply_text(
                f"Désolé, je n'ai rien trouvé pour '{intent['query']}'."
            )
            return
        ...
        if poster:
            ...
            await update.message.reply_text(
                text=caption, parse_mode="Markdown", reply_markup=reply_markup
            )
    else:
    ...
    if query.data == "confirm_add":
        ...
        if success:
            ...
            await query.edit_message_caption(
                caption=f"✅ '{data['item']['title']}'{season_txt} a été ajouté et la recherche est lancée !"
            )
        else:
            await query.edit_message_caption(
                caption="❌ Erreur lors de l'ajout ou média déjà existant."
            )
