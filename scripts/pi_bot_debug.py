...
def admin_main(m):
    if m.from_user.id != SUPER_ADMIN:
        bot.send_message(
            SUPER_ADMIN,
            f"⚠️ **Alerte Sécurité** : Tentative d'accès au menu admin par {m.from_user.id} (@{m.from_user.username})",
        )
        return
...
def admin_router(call):
    ...
    if cmd == "main":
        ...
        markup = InlineKeyboardMarkup(row_width=1)
        markup.add(
            InlineKeyboardButton("✅ Contenu possédé", callback_data="adm:share_owned"),
        ...
        markup = InlineKeyboardMarkup(row_width=1)
        markup.add(
            InlineKeyboardButton("📋 Lister les accès", callback_data="adm:users_list"),
            InlineKeyboardButton(
                "➕ Ajouter un utilisateur", callback_data="adm:users_add"
            ),
        ...
        msg = bot.send_message(
            call.message.chat.id,
            "👤 Envoyez l'ID Telegram de l'utilisateur à ajouter :",
        )
        ...
        bot.edit_message_text(
            "🔍 **Partage de Nouveau Contenu**\n\nEnvoyez-moi le nom du film ou de la série que vous souhaitez rechercher et partager. L'IA s'occupera de la recherche.",
            call.message.chat.id,
            call.message.message_id,
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith(
        ("sel:", "act:", "share_exec:", "cancel", "dl:", "q_refresh")
    )
)
    ...
    if d[0] == "q_refresh":
        ...
        try:
            ...
            bot.answer_callback_query(
                call.id, "❌ Session expirée, relancez la recherche."
            )
            return
        markup = InlineKeyboardMarkup(row_width=2)
        ...
        if call.from_user.id == SUPER_ADMIN:
            markup.add(
                InlineKeyboardButton("🔗 Partager", callback_data=f"share_exec:{d[2]}")
            )
        ...
        if subtitles:
            ...
            markup.add(
                InlineKeyboardButton(
                    "⚡ Lien Rapide (Symlink)", callback_data=f"share_final:{d[1]}:link"
                ),
                InlineKeyboardButton(
                    "🎬 Remux avec Sous-titres",
                    callback_data=f"share_final:{d[1]}:remux",
                ),
                InlineKeyboardButton("❌ Annuler", callback_data="cancel"),
            )
            bot.edit_message_text(
                f"🎯 **Options de Partage**\n{len(subtitles)} sous-titre(s) trouvé(s).",
                call.message.chat.id,
                call.message.message_id,
                reply_markup=markup,
            )
        ...
        bot.edit_message_text(
            "⏳ Traitement en cours..." if remux_opt else "⏳ Génération du lien...",
            call.message.chat.id,
            call.message.message_id,
        )
        ...
        if data and "video" in data:
            ...
            bot.send_message(
                call.message.chat.id,
                "❌ Erreur: Échec du partage ou contenu hors limites.",
            )
    elif d[0] == "cancel":
        bot.edit_message_text(
        ...
        if d[1] == "no":
            ...
            bot.edit_message_text(
                "⏳ Ajout en cours...", call.message.chat.id, call.message.message_id
            )
            ...
            try:
                ...
                if r_add.status_code in [200, 201]:
                    bot.edit_message_text(
                        f"📥 **Ajouté à la file d'attente :** `{dl_data['found_title']} ({dl_data['year']})`\nLe téléchargement va débuter.",
                        call.message.chat.id,
                        call.message.message_id,
                        parse_mode="Markdown",
                    )
                    ...
                    bot.edit_message_text(
                        f"❌ Échec de l'ajout (Erreur {r_add.status_code}).",
                        call.message.chat.id,
                        call.message.message_id,
                    )
            except Exception as e:
...
def films_command(m):
    ...
    try:
        ...
        data = r.json()
        logging.info(
            f"[DEBUG_API] Status: {r.status_code} | Nombre de films reçus: {len(data)}"
        )
...
def series_command(m):
    ...
    try:
        r = requests.get(
            f"{SONARR_CFG['url']}/api/v3/series",
        ...
        text += "\n".join(
            [
                f"• {s['title']} — `{s['statistics']['episodeFileCount']}` épisodes"
                for s in owned
            ]
        )
        bot.reply_to(m, text, parse_mode="Markdown")
    ...
    try:
        ...
        if match:
            ...
            try:
                ...
                parsed["saison"] = (
                    int(parsed.get("saison"))
                    if parsed.get("saison") is not None
                    else None
                )
                parsed["episode"] = (
                    int(parsed.get("episode"))
...
def handle_get_command_entry(m):
    ...
    if len(parts) > 1 and parts[1].strip():
        ...
        msg = bot.reply_to(
            m, "🎬 Que veux-tu télécharger ? (Ex: Shogun S01E01 ou Dune)"
        )
...
def process_get_request(m, query):
    ...
    if not data or "error" in data:
        bot.reply_to(
            m, "❌ Le flux d'analyse a échoué (Pipe Break). Données invalides."
        )
        return
    ...
    bot.send_message(
        m.chat.id,
        f"🔍 **Recherche en cours :** `{titre}` ({type_media.capitalize()})",
        parse_mode="Markdown",
    )
    ...
    try:
        res_local = subprocess.run(
            ["python3", SCRIPT_PATH, "search_local", titre],
            capture_output=True,
            text=True,
        )
        ...
        if local_items:
            bot.send_message(
                m.chat.id,
                f"✅ `{titre}` est déjà disponible localement dans la bibliothèque.",
            )
            return
    except Exception as e:
    ...
    try:
        ...
        r_lookup = requests.get(
            f"{cfg['url']}/api/v3/{endpoint}/lookup?term={titre}",
            headers={"X-Api-Key": cfg["key"]},
            timeout=10,
        )
        ...
        if not results:
            bot.send_message(
                m.chat.id,
                f"❌ `{titre}` introuvable dans les bases de données (TMDb/TVDb).",
            )
            return
        ...
        if exists:
            bot.send_message(
                m.chat.id,
                f"✅ `{found_title} ({year})` est déjà possédé ou en cours de téléchargement sur {'Radarr' if type_media == 'film' else 'Sonarr'}.",
            )
            return
        ...
        try:
            ...
            logging.error(f"Context save failed: {e}")
            bot.send_message(
                m.chat.id, "❌ Erreur interne lors de la préparation du téléchargement."
            )
            return
        ...
        logging.error(f"Arr API Error: {e}")
        bot.send_message(
            m.chat.id, "❌ Erreur de communication avec les services de téléchargement."
        )
